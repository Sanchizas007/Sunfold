...
- [ ] Проверить приложение в трёх языках (Настройки приложения → Язык, переключается на месте)
- [ ] Поднять `CURRENT_PROJECT_VERSION` перед каждой загрузкой

## 10. Собрать пакет отправки — покупки не едут сами

Самая дорогая ошибка проекта: 27.08 версия ушла на проверку **одна**, а три
покупки остались в статусе «Подготовка к отправке». Ревьюер увидел бы пейвол,
где нечего купить, — гарантированный отказ. Замечено только через неделю.

App Store Connect подсказывает это плохо, поэтому порядок по шагам:

1. На странице **каждой** покупки и **каждой** подписки — кнопка
   «Добавить для проверки».
2. Если ASC отвечает «добавьте версию приложения для выбранной платформы» —
   это значит «покупке нужна версия-попутчик», а **не** «загрузите билд».
   Сначала добавьте в черновик версию, потом покупки.
3. **Группа подписок — отдельный объект**, её тоже надо добавить. Иначе:
   «Ваша автоматически возобновляемая подписка должна быть отправлена с
   соответствующей группой подписок». У группы должны быть заполнены и
   **сохранены** локализации — отображаемое название на каждом языке. Окно
   называется «Добавление локализации»; закрытое по «Отменить» ничего не
   сохраняет.
4. Перед Submit открыть черновик и сверить счётчик «Объектов, готовых к
   отправке». Для Sunfold это **5**: версия + Lifetime + две подписки +
   группа. Жёлтой плашки «Не удаётся отправить на проверку» быть не должно.
5. После Submit проверить «Отправленные объекты (N)» — N должно совпасть с
   черновиком. Если нет — отменить отправку и пересобрать.

Отмена отправки безопасна: версия возвращается в редактируемое состояние,
билд остаётся загруженным, заново собирать и заливать ничего не нужно.
