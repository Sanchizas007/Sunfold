...
на вопрос «ведёте ли вы сбор данных» ответ **Да** — **не «Data Not Collected»**.
Apple прямо пишет, что вы отвечаете за любой сторонний код в бинарнике.

Отмечается **ровно один тип данных**:

| Категория | Собирается | Связано с личностью | Для трекинга | Назначение |
|---|---|---|---|---|
| **Purchases** | Да | Нет | Нет | **Analytics + App Functionality** |

⚠️ Уточнено 26.08 по документации RevenueCat. Раньше здесь было записано, что
надо декларировать ещё `Identifiers → User ID` и `Device ID` — **это неверно
для нашего случая**. Они нужны, только если приложение задаёт свой
идентификатор пользователя или использует IDFA. У нас
`Purchases.configure(withAPIKey:)` без `appUserID` и без `logIn`, то есть
идентификатор анонимный, рекламных интеграций нет.

⚠️ Целей у Purchases **две, а не одна**: «Analytics» покрывает панель
RevenueCat (история клиентов, графики, эксперименты), «App Functionality» —
проверку чеков и выдачу доступа.

⚠️ Health НЕ отмечать: вес и история голоданий лежат только на устройстве,
HealthKit не подключён. Анкета про то, что уходит с устройства, а не про то,
что пользователь записывает у себя.

Всё остальное — нет. Tracking — **нет**, ATT-промпт не нужен.

- [x] **Страницы опубликованы и проверены** на GitHub Pages 2026-08-12. Домен
      покупать не требовалось: оба обязательных поля можно заполнить этими
      адресами.

| Поле в App Store Connect | Что вставить |
|---|---|
| **Privacy Policy URL** | `https://sanchizas007.github.io/Sunfold/privacy/` |
| **Support URL** | `https://sanchizas007.github.io/Sunfold/support/` |

Текст политики собирается из той же таблицы строк, что и экраны приложения,
поэтому хостинговая версия дословно совпадает с той, что видит ревьюер внутри
приложения. Деплой автоматический: любой пуш, меняющий `site/`, публикует заново.

> Домен sunfold.app при желании прикручивается позже — Pages это поддерживает.
> Пересобрать сайт без переменных окружения и поменять два поля выше.
...
- [ ] Скриншоты **6.5″ (1284×2778)** — именно этот размер просит форма карточки
      ...
      Tools/screenshots.sh en       # только одна локаль
      ```

      Кадры лягут в `screenshots/<локаль>/`: таймер в процессе, фазы, история
      со стриком, вес с графиком, выбор протокола, настройки. Данные сеет
      `DemoData` (только DEBUG, включается аргументом запуска), даты всегда
      относительно момента съёмки — пересняли и всё снова свежее.
...
## 7a. Снимок экрана для проверки покупок — грабли

У каждой встроенной покупки есть обязательное поле **«Снимок экрана»**.
Снимается командой:

```
Tools/screenshots.sh              # среди экранов есть paywall
```

⚠️ **Размер: 1284×2778 (6.5″), НЕ 1320×2868.** Проверено вживую 25.08.2026:
файл 6.9″ поле не принимает, хотя этот размер есть в официальном списке
скриншотов App Store и для самой карточки приложения годится. Проверка в поле
покупок отстаёт от общего списка. Снять 6.5″ можно на временном симуляторе:

```
xcrun simctl create "tmp-65" com.apple.CoreSimulator.SimDeviceType.iPhone-14-Plus \
  com.apple.CoreSimulator.SimRuntime.iOS-26-5
SUNFOLD_DEVICE="tmp-65" Tools/screenshots.sh en
xcrun simctl delete "tmp-65"
```

⚠️ **Альфа-канал.** `simctl io screenshot` пишет RGBA, App Store Connect такие
картинки не принимает и сообщает об этом как о **«неверных размерах»** — то
есть отправляет искать не ту проблему. `Tools/screenshots.sh` выравнивает
кадры в RGB при 72 dpi сам, но при ручной съёмке про это надо помнить.

⚠️ Поле **«Изображение»** рядом — другое, необязательное, ему нужно ровно
1024×1024. Нужно только для промокодов и продвижения покупки на странице
приложения. Не путать с полем снимка экрана.
